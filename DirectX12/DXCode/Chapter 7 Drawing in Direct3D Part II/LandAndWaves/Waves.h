...
private:
    int mNumRows = 0;//行数 包括边界
    int mNumCols = 0;//列数
    int mVertexCount = 0;//顶点数
    int mTriangleCount = 0;//三角形数
    float mK1 = 0.0f;
    float mK2 = 0.0f;
    float mK3 = 0.0f;
    float mTimeStep = 0.0f;//更新周期
    float mSpatialStep = 0.0f;
    std::vector<DirectX::XMFLOAT3> mPrevSolution;//前坐标
    std::vector<DirectX::XMFLOAT3> mCurrSolution;//坐标
    std::vector<DirectX::XMFLOAT3> mNormals;//法线
    std::vector<DirectX::XMFLOAT3> mTangentX;//切线
};
